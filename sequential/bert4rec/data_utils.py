...
def train_valid_split_last(seqs, min_interactions=3, n_valid=1):
    train_seqs = []
    valid_gt = {}

    for u, s in enumerate(seqs):
        if len(s) < min_interactions or len(s) <= n_valid:
            train_seqs.append(s)
            valid_gt[u] = []
            continue

        train_seqs.append(s[:-n_valid])
        valid_gt[u] = s[-n_valid:]

    return train_seqs, valid_gt
