...
class MetricsCallback(Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        # Log to console
        log.info(
            f"Trial {self.trial.number} | Epoch {current_epoch} | NDCG@10: {val_ndcg:.4f}"
        )
...
class OptunaObjective:
    ...
    def __call__(self, trial: optuna.Trial):
        ...
        log.info(
            f"  Model: hidden={hidden_units}, heads={num_heads}, layers={num_layers}, max_len={max_len}"
        )
        log.info(
            f"  Training: lr={lr:.6f}, weight_decay={weight_decay:.4f}, batch={batch_size}"
        )
        log.info(
            f"  Masking: random={random_mask_prob:.2f}, last_item={last_item_mask_ratio:.2f}"
        )
        log.info("")
        ...
        try:
            ...
            val_nrecall = trainer.callback_metrics.get("val_nrecall@10", 0.0)
            val_hit = trainer.callback_metrics.get("val_hit@10", 0.0)

            if isinstance(best_score, torch.Tensor):
                best_score = best_score.item()
            if isinstance(val_nrecall, torch.Tensor):
                val_nrecall = val_nrecall.item()
            if isinstance(val_hit, torch.Tensor):
                val_hit = val_hit.item()
            ...
            log.info(f"  Current val_nrecall@10: {val_nrecall:.4f}")
            log.info(f"  Current val_hit: {val_hit:.4f}")
            log.info("=" * 80)
            log.info("")
...
def tune_with_monitoring(
        ...
        try:
            ...
            if running_trials:
                ...
                for trial in running_trials:
                    ...
                    log.info(f"  ✓ Marked trial #{trial.number} as FAILED")
        except KeyError:
    ...
    log.info(
        f"     $ sqlite3 {study_name}.db 'SELECT number, value FROM trials ORDER BY value DESC LIMIT 5;'"
    )
    log.info("")
    log.info("=" * 80)
    log.info("")
...
def print_trial_callback(study, trial):
    ...
    if trial.state == optuna.trial.TrialState.COMPLETE:
        ...
        log.info(
            f"  Current best: {study.best_value:.4f} (Trial {study.best_trial.number})"
        )


def main():
    parser = argparse.ArgumentParser(
        description="BERT4Rec Optuna Tuning with Monitoring"
    )
